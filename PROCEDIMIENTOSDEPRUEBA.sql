...
(IN EMPRESA INT)
BEGIN
	SELECT T.ID_TRA, T.NOM_TRA, T.APE_TRA,T.DNI_TRA , E.ID_EMP, E.NOM_EMP, T.EST_TRA
	FROM trabajadores T
...
----- Buses -------
DELIMITER //
DROP PROCEDURE IF EXISTS CAMIONESXEMPT;
CREATE PROCEDURE CAMIONESXEMPT
(IN EMPRESA BIGINT)
BEGIN
	SELECT C.ID_CAM, C.PLACA_CAM, T.ID_TRA, T.NOM_TRA, T.APE_TRA, C.EST_CAM , E.ID_EMP, E.NOM_EMP 
	FROM camiones C
	INNER JOIN trabajadores T ON C.TRA_ID = T.ID_TRA
    INNER JOIN empresas E ON E.ID_EMP = C.EMP_ID
	WHERE C.EMP_ID = EMPRESA
    ORDER BY C.ID_CAM DESC;
END //

CALL CAMIONESXEMPT(1);
...
CALL AGREGAR_DETALLE_BATERIAS(1, '2023-05-25', '15:30:00', 12.5, 60, 20.3, 1);
CALL AGREGAR_DETALLE_BATERIAS(1, '2023-05-25', '16:30:00', 12.5, 60, 20.3, 1);
CALL AGREGAR_DETALLE_BATERIAS(1, '2023-05-25', '17:30:00', 12.5, 60, 20.3, 1);
CALL AGREGAR_DETALLE_BATERIAS(1, '2023-05-25', '18:30:00', 12.5, 60, 20.3, 1);
CALL AGREGAR_DETALLE_BATERIAS(1, '2023-05-25', '19:30:00', 12.5, 60, 20.3, 1);
CALL AGREGAR_DETALLE_BATERIAS(1, '2023-05-25', '20:30:00', 12.5, 60, 20.3, 1);
...
--
CALL AGREGAR_DETALLE_BATERIAS(2, '2023-05-25', '15:30:00', 18.5, 60, 20.3, 1);
CALL AGREGAR_DETALLE_BATERIAS(2, '2023-05-25', '16:30:00', 14.5, 60, 20.3, 1);
CALL AGREGAR_DETALLE_BATERIAS(2, '2023-05-25', '17:30:00', 10.3, 60, 20.3, 1);
--
...
DELIMITER //
DROP PROCEDURE IF EXISTS CAMIONXBATERIA;
CREATE PROCEDURE CAMIONXBATERIA
(IN CAMION INT, IN BATERIA INT)
BEGIN
	SELECT D.id_dc, B.NOM_BAT, C.PLACA_CAM, D.DIA, D.HORA, D.VOLTAJE, D.CARGA, D.TEMPERATURA, D.CORRIENTE
	FROM DETALLE_BATERIAS D
	INNER JOIN BATERIAS B ON B.ID_BAT = D.BAT_ID
	INNER JOIN CAMIONES C ON C.ID_CAM = B.CAM_ID
	WHERE B.CAM_ID = CAMION 
...
DELIMITER //
DROP PROCEDURE IF EXISTS CAMIONXBATERIAT;
CREATE PROCEDURE CAMIONXBATERIAT
(IN CAMION INT, IN BATERIA INT)
BEGIN
	SELECT D.id_dc, B.NOM_BAT, C.PLACA_CAM, D.DIA, D.HORA, D.VOLTAJE, D.CARGA, D.TEMPERATURA, D.CORRIENTE
	FROM DETALLE_BATERIAS D
...
call CAMIONXBATERIAT(1,1);

DELIMITER //
DROP PROCEDURE IF EXISTS BATERIAVALORES;
CREATE PROCEDURE BATERIAVALORES
(IN CAMION INT)
BEGIN
	SELECT * FROM baterias
    WHERE cam_id = CAMION;
